...
void QtWorkbench::BuildMenu(wxMenuBar* menuBar)
{
    ...
    // This menu is generated by the compiler plugin and we don't know if it
    // is already loaded. The problem will be if the compiler plugin is disabled
    // as this while will run forever. Maybe a timeout would be a good idea...
    wxMenu* buildMenu = NULL;
    while(!buildMenu){
        int buildMenuPos = menuBar->FindMenu(_("&Build"));
        buildMenu = menuBar->GetMenu(buildMenuPos);
